import { addFriend, allMyFriends, confirmFriendship, rejectFriend } from "../models/friends.models.js";
...
export const _addFriend = async (req, res) => {
    try {
        ...
        const { user2, alredyFriend, itsme, newFriend, checkFriends, myFriends } = await addFriend({ user_id_1, user_id_2 })
        console.log('USERS CONTROLLERS', user2, alredyFriend, newFriend, checkFriends, myFriends);

        if (itsme) {
            return res.status(201).json({ msg: "You can't add yourself as a friend." })
        }

        if (alredyFriend) {
            return res.status(201).json({ msg: 'You already have this request.' })
        }
...
export const _confirmFriendship = async (req, res) => {
    ...
    try {
        ...
        const myNewFriendsList = await confirmFriendship({ user_id_1, user_id_2 })
